...
├── test/run.js               의존성 없는 테스트 러너 (99 케이스)
...
| `.pdf` | `pdfjs-dist` 로 추출 (한글 CID 폰트의 ToUnicode CMap 해석이 필요해 직접 파싱하지 않음). 페이지마다 반복되는 머리글·바닥글, 페이지 번호·인쇄 시각 줄은 자동 제거. **줄바꿈으로 끊긴 문장은 다시 이어 붙입니다** (아래 참고) |
...
> **PDF 의 줄바꿈** — PDF 는 **시각적 줄**만 알려줍니다. 한 문장이 두 줄에 걸치면 두 조각이 되고,
> 파서가 각 조각을 별개 요구사항으로 읽어 `…기능 설정 (이` 처럼 **단어 중간에서 끊긴** 요구사항이 생깁니다.
> 그래서 **줄이 꽉 찼는데 문장이 안 끝났으면** 다음 줄과 잇습니다. "꽉 찼는지" 는 그 페이지에서 흔한
> 줄 길이를 기준으로 판단하므로, 일부러 짧게 끊은 줄(제목·목록·표 셀)은 이어붙지 않습니다.
>
> | 입력(PDF 의 두 줄) | 결과 |
> |---|---|
> | `…기능 설정 (이` + `때 비밀번호는…` | `…기능 설정 (이때 비밀번호는…` |
> | `…제공일로부터 12` + `개월간 보관…` | `…제공일로부터 12개월간 보관…` |
> | `…any con-` + `figuration changes…` | `…any configuration changes…` |
>
> 한글은 단어 중간에서도 줄이 바뀌므로 공백 없이 붙이고, 영문은 단어 사이에서 바뀌므로 공백을 넣습니다.
> 하이픈 분철은 하이픈을 지우고 잇습니다.

> **기호 없는 제목** — PDF·워드에서 뽑은 텍스트에는 마크다운 기호가 없습니다.
> `개요 (Introduction)` 같은 국·영문 병기 제목과 표준 절 이름(개요 · 요약 · 배경 · 목표 · 범위 ·
> 제약사항 …)을 제목으로 인식합니다. 이게 없으면 문서 전체가 **한 영역**으로 뭉쳐
> 영역 필터와 검증 분석서가 무의미해집니다.
...
### 테스트 값 · 조건 지정 → TC 다시 생성

분석 결과 아래 **`테스트 값 · 조건 지정`** (기본 접힘)을 펴면 두 칸을 넣을 수 있습니다.
안 넣어도 TC 는 이미 만들어져 있으므로, 필요한 사람만 펴서 씁니다.

| 화면에서 지정 | 만들어지는 것 |
|---|---|
| 필드별 **정상 테스트 값** | TC 수행 단계에 실제 값이 그대로 들어감 + **`실제로 제출해 확인` 의 유일한 입력원** |
| 폼별 **선행 조건** (예: `로그인한 회원만 접근 가능`) | 조건 충족 상태의 정상 제출 + **조건 미충족 시 차단되는지** (`negative-condition`) |

- QA 가 지정한 값이 페이지 관측값보다 **우선**합니다 (실제 규칙을 아는 쪽이 QA 이므로).
- 재생성은 **페이지를 다시 가져오지 않습니다.** 값을 고쳐가며 몇 번을 눌러도 네트워크·레이트리밋을
  소모하지 않고, 지정한 값은 재생성 왕복에서 유지됩니다.
- `초기화` 는 처음 관측한 상태로 되돌립니다.

> **화면에서 뺀 칸들** — 처음에는 필수 여부 · 최소/최대 길이 · 형식 규칙 · 조건·비고 · 필드 추가까지
> 6칸이었는데, 입력 부담만 크고 대부분 TC 를 한 건씩 늘리는 수준이라 화면에서 뺐습니다.
> 최대 길이는 HTML 에 이미 있는 경우가 많고(naver 검색창의 `maxlength=255` 는 자동 인식),
> 필드 추가는 위 **브라우저 렌더링 분석**이 생기면서 대부분 필요 없어졌습니다.
>
> **서버는 그대로 처리하므로 API 로는 여전히 지정할 수 있습니다** — 아래 `overrides` 의
> `required` · `minLength` · `maxLength` · `rule` · `note` · `addedFields`. 각각
> 경계값 ±1 · `qa-rule` · `qa-note` TC 를 만듭니다. 대량으로 조건을 넣어야 하면 스크립트로 쓰세요.
