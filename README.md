...
│   ├── questions.js          요청 확인 항목 — 기획·개발에 물어볼 것 (화면·연동 중심)
│   ├── web/
...
│   │   ├── crawl.js          로그인 후 사이트 탐색 (위험 링크 회피·상한)
│   │   ├── siteTestCases.js  탐색 결과 → 로그인·딥링크·화면별 TC
...
### 로그인 후 사이트 탐색 — 화면마다 세부 TC

위 한 장짜리 분석은 준 URL **한 화면**만 봅니다. 그래서 내부 링크가 몇 개든
`주요 내부 링크 이동 (N개 중 대표)` **TC 한 줄**로 뭉치고, 그 링크 너머의
주문내역·쿠폰·설정 화면은 아예 분석되지 않았습니다.

`웹사이트 분석` 탭의 **`로그인해서 여러 화면 분석`** 을 펴면 링크를 따라가며
**화면마다** TC 를 만듭니다. 로그인 정보를 주면 그 세션으로 돌기 때문에
**로그인 뒤에만 보이는 화면**까지 분석됩니다.

목(mock) 로그인 사이트 실측 — 같은 사이트, 같은 시작 주소:
...
| 묶음 | 내용 |
|---|---|
| **로그인** | 정상 로그인(이동 화면 실측) · 잘못된 비밀번호 · 세션 유지(새로고침·새 탭) |
| **딥링크** | 주소 직접 입력으로 진입 · **비로그인 상태 직접 접근 시 차단·복귀** · 잘못된 경로/파라미터 |
| **화면 이동** | 메뉴로 이동·뒤로 가기 · 위험해서 건너뛴 링크 수동 확인 · 상한으로 못 본 화면 |
| **화면별** | 화면마다 폼·입력 제약·경계값·XSS — 기존 한 장 분석 로직을 화면 단위로 재사용 |

반응형·접근성·예외 처리처럼 화면이 바뀌어도 같은 TC 는 **한 번만** 냅니다
(화면 10개면 같은 줄이 40개 나옵니다).

```jsonc
POST /api/analyze-site
{
  "url": "https://shop.example.com/",
  "login": { "username": "qa-account", "password": "…", "url": "https://shop.example.com/login" },
...
남의 서비스를 자동으로 돌아다니는 일이라 좁게 잠가 뒀습니다.

- **링크 이동(GET)만** 합니다. 탐색 중에는 폼을 제출하지 않습니다(로그인 폼 제외).
- **로그아웃·삭제·탈퇴·해지·초기화**로 보이는 링크는 따라가지 않고 **수동 확인 TC** 로 남깁니다.
  크롤러가 로그아웃을 밟으면 이후 탐색이 전부 망가지고, 최악의 경우 데이터를 지웁니다.
- **같은 출처(origin)만** 돕니다. 외부 도메인은 목록에만 남습니다.
- 페이지 수·깊이 상한(기본 10개 · 깊이 2, 최대 30개)을 둡니다. 못 본 화면은 숨기지 않고 TC 로 알립니다.
- 로그인은 대상 사이트에 **실제 자격 증명을 보내는 행위**라 실행 검증과 **같은 3중 게이트**를 통과해야 합니다
  (`SPECTOTC_LIVE_SUBMIT` + `SPECTOTC_LIVE_ALLOW_HOSTS` + `SPECTOTC_LIVE_ALLOW_POST`).

#### 자격 증명 취급

받은 아이디·비밀번호는 **브라우저에 입력하는 데만** 쓰고 어디에도 남기지 않습니다.

- 로그에 남지 않습니다 — 시도 여부와 성공 여부만 기록합니다
- 응답으로 돌려주지 않습니다
- **TC 문구에 들어가지 않습니다** — `전달받은 테스트 계정` 자리표시자로 적습니다
  (들어가면 CSV·PDF 로 그대로 새어 나갑니다. 테스트로 고정했습니다)
- 화면에서도 실행 직후 비밀번호 칸을 비웁니다

#### 사내 스테이징(내부망) 주소

SSRF 방어 때문에 `localhost`·사내 IP 는 기본적으로 막힙니다. 스테이징이 내부망에만 있으면
아래 두 조건을 **모두** 갖춰야 열립니다 — "아무 내부 주소나" 가 아니라 "적어 둔 그 주소만" 입니다.

```bash
SPECTOTC_ALLOW_INTERNAL=1
SPECTOTC_LIVE_ALLOW_HOSTS=staging.사내도메인,10.0.1.20   # 여기 적힌 호스트만
```

공개 URL 분석(`/api/analyze-url`)에는 적용되지 않습니다. **브라우저 탐색 경로 전용**입니다.
...
| `title` | `[경계] 경계값 8글자 이상 전후 판정 — 비밀번호는 8자 이상 20자 이하로 입력해야 한다` — 유형 · 무엇을 확인 · 어느 요구사항 대상인지가 한 줄에 (아래 참고) |
...
**수행 단계의 주어는 QA, 기대 결과의 주어는 시스템**입니다.

처음에는 요구사항의 동작절을 그대로 `실행:` 단계에 넣었습니다. 그런데 동작절은 **시스템이 하는 일**입니다.
`실행: 홈 화면으로 이동하고, 액세스 토큰을 저장한다` 는 QA 가 할 수 있는 행동이 아니라서,
읽는 사람이 무엇을 해야 하는지 알 수 없었습니다.

```
수행 단계  1. 진입: QA 는 로그인 화면에 진입한다
          2. 조건 설정: QA 는 로그인 성공 상태를 만든다
          3. 실행: QA 는 로그인 화면에서 해당 기능을 실행한다 (기대 동작: 홈 화면으로 이동하고…)
          4. 확인: QA 는 화면에 표시된 결과와 서버 응답을 확인한다
기대 결과  • 시스템은 홈 화면으로 이동하고, 액세스 토큰을 저장한다
...
### 제목 규칙 — 한 줄만 보고 무슨 테스트인지 알 수 있게

`[유형] 무엇을 확인 — 어느 요구사항 대상`

| | 이전 | 지금 |
|---|---|---|
| 실패 TC | `[실패] 1. 로그인 — 미인증 / 권한 없는 계정 접근` | `[실패] 미인증 / 권한 없는 계정 접근 — 비밀번호는 8자 이상 20자 이하로…` |

- **영역을 제목에서 뺐습니다.** 화면 표와 CSV 모두 `영역` 이 별도 칸이라 그대로 중복이었습니다.
- **대상 요구사항을 붙였습니다.** `미인증 / 권한 없는 계정 접근` 같은 레시피 제목은 한 영역 안에서
  요구사항마다 똑같이 나와, 표에서 네 줄이 전혀 구별되지 않았습니다. 이제 샘플 기획서 98건에
  **완전히 같은 제목이 없습니다** (테스트로 고정).
- 제목이 이미 대상을 포함하면(정상 흐름 등) 덧붙이지 않습니다.
...
| **확인 필요** | 문서가 **틀렸거나 모호한 곳** — 모호 표현(`적절히`, `필요시`, `등`, `빠르게`, `TBD`), 재시도 횟수 누락, 단위 누락, 조건만 있고 실패 처리 없음, 결제에 실패 흐름 없음. 사유별로 묶고 **질문 문장까지** 생성 |
| **요청 확인 항목** | 문서가 맞아도 **이게 없으면 테스트를 못 하는 것** — 아래 참고 |
| **영역별 요점** | 영역마다 요구사항 수·주요 카테고리·대표 문장 3건 |

### 요청 확인 항목 — 기획·개발에 물어볼 것
...
**화면(FE)과 연동에 무게를 둡니다.** 백엔드 로직은 대개 문서에 적히지만, 로딩·빈 상태·에러 문구·
타임아웃 화면·서드파티 샌드박스처럼 **화면과 연동 경계에서 벌어지는 일**은 문서에 거의 없고
그게 QA 를 멈춰 세웁니다. 샘플 기획서 기준 화면·연동 비중 **83%** (23건 중 High 12건).
...
`요청 확인 항목 복사` 버튼으로 마크다운을 받아 그대로 전달하고, **전체 문서 PDF** 에도 표로 들어갑니다.
...
컬럼 21개: `연번, 요구사항 영역, 영역 내 순서, TC_ID, TC 유형, 중요도, 수행 결과, 수행일, 담당자, 비고, 테스트 시나리오, 검증 목적, 사전 조건, 수행 단계, 기대 결과, 요구사항 ID, 근거 문장, 원문 라인, 분류, 태그, 생성 방식`

**`TC 유형` 은 수행 결과가 아닙니다.** 예전에는 이 칸에 `Pass` / `Fail` 을 그대로 넣었는데,
아직 아무것도 실행하지 않았는데 이미 "Pass" 라고 적혀 있어 **결과로 읽혔습니다.**
이제 `정상` · `실패` · `경계` 로 적고, 실제 결과는 비워 둔 `수행 결과` 칸에 QA 가 채웁니다.
`수행일` · `담당자` · `비고` 도 빈 칸으로 함께 나갑니다.

| TC 유형 | 뜻 |
|---|---|
| `정상` | 명세대로 동작해야 하는 흐름 (happy path) |
| `실패` | 차단·거부되어야 하는 흐름 |
| `경계` | 경계값·중복 실행·대량 데이터 같은 가장자리 |

사전 조건·기대 결과는 셀 안에서 `•` 불릿, 수행 단계는 `1. 2. 3.` 번호로 줄바꿈됩니다. 양식 변경은 `src/csv.js` 의 `COLUMNS` 배열만 수정하면 됩니다.

**영역별로 묶어 정렬합니다.** 생성 순서 그대로 내보내면 같은 영역의 정상·실패·경계가 흩어져,
Excel 에서 "이 영역에 TC 가 몇 건이고 무엇을 덮는지" 를 볼 수 없었습니다.
...
| `로그인-17` | 1. 로그인 | `17/24` | Edge Case | [경계] 연속 중복 실행(따닥) 및 처리 중 새로고침 — … |
| `회원가입-01` | 2. 회원가입 | `1/23` | Pass | [정상] 필수 약관에 모두 동의해야 가입을 완료할 수 있다 |

- 정렬 기준: **영역(문서 등장 순서) → 유형(정상 → 실패 → 경계) → 중요도(High → Med → Low) → TC_ID**
- 영역 순서는 **문서에 나온 순서**를 유지합니다. 가나다순으로 다시 세우면 기획서를 읽은 순서와 어긋나 대조가 어려워집니다.
- `영역 내 순서` 의 `8/24` 로 이 영역의 전체 분량과 현재 위치가 한눈에 보입니다.
